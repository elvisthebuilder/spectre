...
class Client:
    ...
    def create_account(self, cookies):
        ...
        # Step 1: Seed the CSRF token by hitting the csrf endpoint
        try:
            csrf_resp = self.session.get("https://www.perplexity.ai/api/auth/csrf", timeout=10)
            csrf_data = csrf_resp.json()
            csrf_token = csrf_data.get("csrfToken")
            if not csrf_token:
                raise Exception(f"CSRF token not found in response: {csrf_resp.text[:200]}")
            logger.info(f"CSRF token obtained: {csrf_token[:8]}...")
        ...
        while attempts < 3:
            ...
            try:
                ...
                logger.info(f"Attempt {attempts}: Using disposable email: {emailnator_cli.email}")
                ...
                logger.info(f"Sign-in request status: {resp.status_code}")
                ...
                if resp.ok:
                    ...
                    if new_msgs:
                        logger.info("Sign-in email received. Completing authentication...")
                        break
                    else:
                        logger.warning("Sign-in email not received within timeout.")
                else:
                    logger.warning(f"Sign-in request failed: {resp.status_code} - {resp.text[:200]}")

            except Exception as e:
                logger.error(f"Account creation attempt {attempts} failed: {e}")
        
        if not emailnator_cli or attempts >= 3:
            raise Exception("Perplexity account rotation failed after 3 attempts.")

        # Extract the sign-in link from the email
        msg = emailnator_cli.get(func=lambda x: x["subject"] == "Sign in to Perplexity")
        if not msg:
            raise Exception("Could not retrieve sign-in email from inbox.")
            
        email_body = emailnator_cli.open(msg["messageID"])
        match = self.signin_regex.search(email_body)
        if not match:
            raise Exception("Sign-in link not found in email body.")
            
        new_account_link = match.group(1).replace("&amp;", "&")
        ...
        self.copilot = 5
        self.file_upload = 10
        logger.info("New Perplexity identity established. Copilot quota: 5")
