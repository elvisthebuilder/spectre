...
class Emailnator:
    ...
    def __init__(
        ...
        # Generate a new email address (max 5 attempts)
        resp = None
        for attempt in range(5):
            try:
                r = self.s.post(EMAILNATOR_GENERATE_ENDPOINT, json=data, timeout=10)
                resp = r.json()
                if "email" in resp:
                    break
                resp = None
        ...
        if not resp or "email" not in resp:
            raise Exception(f"Emailnator failed to generate email after 5 attempts. Last response: {resp}")
